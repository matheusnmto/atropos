...
function register(ipcMain, getStatusFn, runNowFn, store) {
  ...
  // ── Active / Immune note lists ──────────────────────────────────────────────
  ipcMain.handle('notes:active', async () => {
    if (!store) return [];
    const vaultPath = store.get('vaultPath');
    if (!vaultPath) return [];
    try {
      const { getActiveNotes } = require(path.join(__dirname, '../../zelador/modules/metrics'));
      return await getActiveNotes(vaultPath);
    } catch (err) {
      console.error('[ipc] notes:active error:', err);
      return [];
    }
  });

  ipcMain.handle('notes:immune', async () => {
    if (!store) return [];
    const vaultPath = store.get('vaultPath');
    if (!vaultPath) return [];
    try {
      const { getImmuneNotes } = require(path.join(__dirname, '../../zelador/modules/metrics'));
      return await getImmuneNotes(vaultPath);
    } catch (err) {
      console.error('[ipc] notes:immune error:', err);
      return [];
    }
  });

  ipcMain.handle('notes:immunize', async (_, filePath) => {
    try {
      const matter = require('gray-matter');
      const raw = fs.readFileSync(filePath, 'utf-8');
      const parsed = matter(raw);
      parsed.data.decay_immune = true;
      const output = matter.stringify(parsed.content, parsed.data);
      fs.writeFileSync(filePath, output, 'utf-8');
      return { success: true };
    } catch (err) {
      console.error('[ipc] notes:immunize error:', err);
      return { success: false, error: err.message };
    }
  });

  ipcMain.handle('notes:remove-immunity', async (_, filePath) => {
    try {
      const matter = require('gray-matter');
      const raw = fs.readFileSync(filePath, 'utf-8');
      const parsed = matter(raw);
      delete parsed.data.decay_immune;
      const output = matter.stringify(parsed.content, parsed.data);
      fs.writeFileSync(filePath, output, 'utf-8');
      return { success: true };
    } catch (err) {
      console.error('[ipc] notes:remove-immunity error:', err);
      return { success: false, error: err.message };
    }
  });
    ...
    try {
      ...
      for (const { full, rel } of files) {
        ...
        nodes.push({ 
          ...
          name, 
          phase, 
          immune: !!data.decay_immune && data.status !== 'fossilized', 
          filePath: full 
        });
